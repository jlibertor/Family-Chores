...
- **Distinct-child daily unlocks (chore clock, revised 2026-07-03).** Story
  progress is **shared by the whole household** (the pointer is the MAX
  `unlocked_index` across active children). **Each unique child who completes at
  least one chore today unlocks one scene for the family** — the trigger is that
  child's *first* chore of the day; extra chores by the same child do not unlock
  more scenes. With N active children, at most N scenes can unlock per day, and
  the released frontier still caps everything, so nobody can run ahead of the
  release calendar.

Net effect: the story only moves forward, paced sustainably, and **doing chores is
the only way to move it** — and because one kid grinding chores can't carry the
story alone, every child's participation matters ("scene 3 won't unlock until YOU
do a chore too"). Everyone starts with scene 1 visible (`unlocked_index` defaults
to 1); the first child's first chore unlocks scene 2.
...
daughter; Bianca (angelfish) = the friend who proposes the trip; Otto (pufferfish) = the
dad's old contact; Stella (starfish) = the clue-reader; **the bugs / a `stranger`
...
- Helpers: `getActiveStorySeries`, `getStorySceneRows`, `getHouseholdUnlockedIndex`
  (MAX `unlocked_index` across active children — household-shared progress),
...
- **`getStoryForMember(db, memberId)`** — gated player payload `{ series, releasedCount,
  ...
  `accessibleCount = min(releasedCount, max(householdUnlocked, 1))`. `memberId` is
  accepted for API compatibility but progress is household-shared.
- **`advanceStoryProgressForCompletion(db, memberId)`** — called in `recordCompletion`
  for **child members only** (wrapped in `.catch`, non-fatal). Unlocks one scene for
  the household when this is the completing child's **first chore of the day**
  (later chores by the same child do nothing), **capped at the released frontier**
  (`releasedCount`), via `INSERT ... ON CONFLICT` upsert with the new index computed
  in-statement (race-tolerant).
...
- **Migrations `0025` + `0026` verified against a real SQLite dry-run (2026-07-03).**
  Applying them locally and remotely is the remaining actionable step — see
  [`handoff/open/story_engine_handoff.md`](../handoff/open/story_engine_handoff.md).
- **The release-calendar frontier is NOW ENFORCED in code (2026-07-03).** Earlier
  builds computed `release_offset_days` but did not actually cap unlocks with it;
  both `getStoryForMember` and `advanceStoryProgressForCompletion` now clamp to
  `releasedCount`, so nobody can read or unlock past the calendar frontier.
- **Advancement rule revised (2026-07-03, per Jeremy):** household-shared progress;
  **each unique child's first chore of the day unlocks one scene** (max N scenes/day
  with N active children), capped at the released frontier, **child members only**
  (an adult completing a chore does not move the story pointer).
